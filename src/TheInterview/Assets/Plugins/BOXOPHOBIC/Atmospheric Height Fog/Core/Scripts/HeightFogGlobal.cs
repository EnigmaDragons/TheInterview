...
public class HeightFogGlobal : StyledMonoBehaviour
{
    ...
    [StyledMessage("Info", "Shader keyword switching is not updated in realtime in edit in newer Unity versions. Move a slider or save the scene to update the fog until the issue is fixed (if fixable).", 0, 0)]
